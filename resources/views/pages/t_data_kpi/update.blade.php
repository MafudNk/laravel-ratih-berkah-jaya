...
<div class="content">
  ...
  <div class="block block-rounded">
    ...
    <div class="block-content block-content-full">
      ...
      <form class="js-validation" action="{{ route('data_kpis.save_update_isi_kpi', $data_kpi_id->transaksi_kpi_id) }}" method="POST">
        @csrf
        @method('patch')
        <div class="block block-rounded">
              ...
              <table class="table table-bordered table-striped table-vcenter">
                <thead>
                  <tr>
                    ...
                    <th class="text-center">Komentar User</th>
                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      <select class="form-control" name="data_{{$key}}" id="data_{{$key}}" required onchange="getSelectedValue({{ $dt->bobot }}, {{$key}})">
                        ...
                        <option value="0" {{ $dt->nilai == 0 ? 'selected' : '' }} >0</option>
                        <option value="5" {{ $dt->nilai == 5 ? 'selected' : '' }} >5</option>
                        <option value="10" {{ $dt->nilai == 10 ? 'selected' : '' }} >10</option>
                        <option value="15" {{ $dt->nilai == 15 ? 'selected' : '' }} >15</option>
                    ...
                    <td> <input type="text" class="form-control" readonly name="skor_{{$key}}" id="skor_{{$key}}" value="{{$dt->skor}}"></td>
                    <td>{{$dt->komentar_user}}</td>
                    <td> <textarea name="komentar_user_penilai_{{$key}}" id="" cols="10" rows="3">{{$dt->komentar_user_penilai}}</textarea> </td>
                    @endforeach
                  </tr>
                </tbody>
              </table>
